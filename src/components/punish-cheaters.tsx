	...
	if (punishmentUsedThisRound) {
		return (
			<div className="rounded-lg border-2 border-gray-400 bg-gray-100 p-4 text-center">
				<p className="text-sm text-gray-600">{config.punishmentUsedMessage}</p>
			</div>
		);
	}
				...
				<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
					<div className="w-full max-w-md rounded-lg bg-white p-6 shadow-xl">
						...
						<div className="flex gap-2">
							...
							>
								{config.confirmButton} ({selectedPlayers.size})
							</button>
						</div>
					</div>
				</div>
